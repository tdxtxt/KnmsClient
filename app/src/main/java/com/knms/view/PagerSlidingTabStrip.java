...
public class PagerSlidingTabStrip extends HorizontalScrollView {
    ...
    private int indicatorColor = KnmsApp.getInstance().getResources().getColor(R.color.tab_line);
    private int underlineColor = 0x1A000000;
    private int dividerColor = Color.WHITE;//分割线颜色

    //该属性表示里面的TAB是否均分整个PagerSlidingTabStrip控件的宽,
    // true是,false不均分,从左到右排列,默认false
    private boolean shouldExpand = false;
    private boolean textAllCaps = true;//转大写
    private boolean showSelectedTabBg = false;//选中tab背景颜色,ture显示，false不显示
    ...
    private int selectedTabTextColor = Color.parseColor("#333333");
    private int selectedTabBgColor = KnmsApp.getInstance().getResources().getColor(R.color.tab_line);
    ...
    private float zoomMax = 0.0f;//选中放大特效
    ...
    public PagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
        ...
        tabTextColor = a.getColor(R.styleable.PagerSlidingTabStrip_pstsUnderTextColor, tabTextColor);
...
//        indicatorWrap = a.getBoolean(R.styleable.PagerSlidingTabStrip_pstsIndicatorWrap, indicatorWrap);
            ...
            for (int j = 0; j < frameLayout.getChildCount(); j++) {
                ...
                if (v instanceof TextView) {
                    ...
                    if (j == 0) {
                        ...
                        if(showSelectedTabBg) tab.setBackgroundColor(selectedTabBgColor);
                    }
    ...
    public void setShowSelectedTabBg(boolean showSelectedTabBg) {
        this.showSelectedTabBg = showSelectedTabBg;
    }
