 ...
 * 类描述：
 * 创建人：Administrator
 * 创建时间：2017/9/12 10:22
 * 传参：
 * 返回:
 */
public class SpWheelView  extends ScrollView {
...
//    private ScrollView scrollView;
    ...
    public SpWheelView(Context context) {
        super(context);
        init(context);
    }

    public SpWheelView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public SpWheelView(Context context, AttributeSet attrs, int defStyle) {
...
//        this.setOrientation(VERTICAL);
        ...
        this.addView(views);
//        OverScrollDecoratorHelper.setUpStaticOverScroll(views,OverScrollDecoratorHelper.ORIENTATION_VERTICAL);
...
//                    Log.d(TAG, "remainder: " + remainder + ", divided: " + divided);
                    if (remainder == 0) {
                        ...
                        if (remainder > itemHeight / 2) {
                            SpWheelView.this.post(new Runnable() {
                                @Override
                                public void run() {
                                    SpWheelView.this.smoothScrollTo(0, initialY - remainder + itemHeight);
                            ...
                            SpWheelView.this.post(new Runnable() {
                                @Override
                                public void run() {
                                    SpWheelView.this.smoothScrollTo(0, initialY - remainder);
                                    selectedIndex = divided + offset;
                                    onSeletedCallBack();
                                }
                            });
                    ...
                    initialY = getScrollY();
                    SpWheelView.this.postDelayed(scrollerTask, newCheck);
                }
            }
        };
...
//            position = divided + offset + 1;
    ...
    public void setSeletion(int position) {
        ...
        this.post(new Runnable() {
            @Override
            public void run() {
                SpWheelView.this.smoothScrollTo(0, p * itemHeight);
            }
        });
