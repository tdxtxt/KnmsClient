...
public class OnekeyShare {
	...
	public void show(Context context, ShareEntity shareEntity){
		...
		setTitle(shareEntity.getTitle());
		//仅在人人网和QQ空间使用
		setTitleUrl(shareEntity.getUrl());
		//所有平台都需要这个字段
		setText(shareEntity.getRemark());
		//新浪微博分享网络图片需要通过审核后申请高级写入接口，否则请注释掉测试新浪微博
		setImageUrl(shareEntity.getImg());
		//仅在微信（包括好友和朋友圈）中使用
		setUrl(shareEntity.getUrl());
		//分享此内容的网站名称，仅在QQ空间使用
		setSite("凯恩买手");
		//分享此内容的网站地址，仅在QQ空间使用
		setSiteUrl("http://down.kebuyer.com/");
		show(context);
	}
